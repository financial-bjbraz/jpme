...
public class RHjpmeGrauInst extends BaseScreenPainel implements BaseScreenFactory{
	...
	@Autowired
	private GrauDeInstrucaoService service;
	
	public RHjpmeGrauInst() {
...
//		add(pnl1, BorderLayout.NORTH);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource() == btBuscar){
			...
			if(!Util.isBlankOrNull(textoInclusao.getText())){
				...
				grau.setDscrGrauInstr(textoInclusao.getText());
	...
	public class MyTableModel extends AbstractTableModel {
		...
		private Object[][] obterDados() {
			List<GrauDeInstrucao> its = service.listarTodos();
			data                      = new Object[its.size()][columnNames.length];
			
			if(its != null && its.size() > 0){
			
				for (int i = 0; i < its.size(); i++) {
					GrauDeInstrucao g = its.get(i);
					data[i][0] = g.getIdGrauInstr().toString();
					data[i][1] = g.getDscrGrauInstr();
					data[i][2] = g.getInAtiv() == 1 ? "Ativo" : "Inativo";
