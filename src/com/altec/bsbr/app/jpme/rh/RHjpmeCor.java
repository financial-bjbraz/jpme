...
public class RHjpmeCor extends BaseScreenPainel implements BaseScreenFactory{
...
//		add(pnl1, BorderLayout.NORTH);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource() == btBuscar){
			...
			if(!Util.isBlankOrNull(textoInclusao.getText())){
				Cor grau = new Cor();
				grau.setIdAtlz(getUsuario().getCodigoUsuario());
				grau.setIdIncl(getUsuario().getCodigoUsuario());
				grau.setDhAtlz(Util.hojeDataHoraTimesTamp());
				grau.setDhIncl(Util.hojeDataHoraTimesTamp());
				grau.setNmCor(textoInclusao.getText());
				service.salvar(grau);
				constroiPainelResultados();
	...
	public class MyTableModel extends AbstractTableModel {
		...
		private Object[][] obterDados() {
			List<Cor> its = service.listarTodos();
			...
			if(its != null && its.size() > 0){
				...
				for (int i = 0; i < its.size(); i++) {
					Cor g = its.get(i);
					data[i][0] = g.getIdCor().toString();
					data[i][1] = g.getNmCor();
					data[i][2] = "Ativo";
					data[i][3] = g.getIdIncl().toString();
					data[i][4] = Util.dateToStringDDMMYYYmmss(g.getDhIncl());
					data[i][5] = g.getIdAtlz().toString();
