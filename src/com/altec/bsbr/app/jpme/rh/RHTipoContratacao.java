...
import com.altec.bsbr.app.jpme.rh.RHjpmeGrauInst.MyTableModel;
import com.altec.bsbr.app.jpme.service.TipoAdmissaoService;

public class RHTipoContratacao extends BaseScreenPainel implements
		BaseScreenFactory {

	/**
	 * 
	 */
	private static final long serialVersionUID = -3545217231642958074L;
	...
	private JLabel lblusuarioInclusao = new JLabel("Usuário Inclusão :");
	private JLabel lblusuarioAlteracao = new JLabel("Usuário Alteração :");
	private JLabel lbldataInclusao = new JLabel("Data Inclusão :");
	private JLabel lbldataAlteracao = new JLabel("Data Alteração :");
	private JPanel painelResultados = new JPanel(new BorderLayout());
	private JPanel painelInclusao = new JPanel(new BorderLayout());
	private JPanel painelBusca = new JPanel(new BorderLayout());
	...
	private TextField textoInclusao = new TextField(100);
	...
	public RHTipoContratacao() {
		super(new BorderLayout());
		btBuscar.addActionListener(this);

		JPanel pnl1 = Util.panelBox(new Component[] {
				new JLabel("Descrição : ", JLabel.RIGHT), null },
				new Component[] { descricao, btBuscar });
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == btBuscar) {
			...
			if (!Util.isBlankOrNull(textoInclusao.getText())) {
				TipoAdmissao ta = new TipoAdmissao();
				ta.setIdUsrIncl(getUsuario().getCodigoUsuario());
				ta.setIdUsrAtlz(getUsuario().getCodigoUsuario());
				ta.setDtAtlz(Util.hojeDataHoraTimesTamp());
				ta.setDtIncl(Util.hojeDataHoraTimesTamp());
				ta.setNmTipoAdmissao(textoInclusao.getText());
				tipoAdmissaoService.salvar(ta);
				constroiPainelResultados();
	...
	public class MyTableModel extends AbstractTableModel {
		...
		private Object[][] obterDados() {
			List<TipoAdmissao> its = tipoAdmissaoService.listarTodos();
			data = new Object[its.size()][columnNames.length];

			if (its != null && its.size() > 0) {

				for (int i = 0; i < its.size(); i++) {
					TipoAdmissao g = its.get(i);
					data[i][0] = g.getIdTipoAdmissao().toString();
					data[i][1] = g.getNmTipoAdmissao();
					data[i][2] = "Ativo";
					data[i][3] = g.getIdUsrIncl().toString();
					data[i][4] = Util.dateToStringDDMMYYYmmss(g.getDtIncl());
					data[i][5] = g.getIdUsrAtlz().toString();
					data[i][6] = Util.dateToStringDDMMYYYmmss(g.getDtAtlz());
				}
		...
		public void setValueAt(Object value, int row, int col) {
			...
			if (!Util.isBlankOrNull(descr)) {
				...
				try {
					...
					grau = tipoAdmissaoService.findById(id);
					grau.setNmTipoAdmissao(descr);
					grau.setIdUsrAtlz(getUsuario().getCodigoUsuario());
					grau.setDtAtlz(Util.hojeDataHoraTimesTamp());
					tipoAdmissaoService.salvar(grau);
				...
				JOptionPane
						.showMessageDialog(
								painel,
								"Não foi possível salvar o Grau de Instrução. É necessário informar a descrição");
			}
