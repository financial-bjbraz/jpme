...
public class RHjpmeVinculo extends BaseScreenPainel implements BaseScreenFactory{
...
//		add(pnl1, BorderLayout.NORTH);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource() == btBuscar){
			...
			if(!Util.isBlankOrNull(textoInclusao.getText())){
				...
				grau.setNmVinculo(textoInclusao.getText());
	...
	public class MyTableModel extends AbstractTableModel {
		...
		private Object[][] obterDados() {
			List<Vinculo> its = service.listarTodos();
			...
			if(its != null && its.size() > 0){
				...
				for (int i = 0; i < its.size(); i++) {
					Vinculo g = its.get(i);
					data[i][0] = g.getIdVinculo().toString();
					data[i][1] = g.getNmVinculo();
