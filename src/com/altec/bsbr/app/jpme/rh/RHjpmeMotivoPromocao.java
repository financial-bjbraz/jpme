...
public class RHjpmeMotivoPromocao extends BaseScreenPainel implements BaseScreenFactory{
	...
	@Autowired
	private MotivoPromocaoService service;
	
	public RHjpmeMotivoPromocao() {
...
//		add(pnl1, BorderLayout.NORTH);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource() == btBuscar){
			...
			if(!Util.isBlankOrNull(textoInclusao.getText())){
				MotivoPromocao grau = new MotivoPromocao();
				...
				grau.setNmMotivoPromocao(textoInclusao.getText());
	...
	public class MyTableModel extends AbstractTableModel {
		...
		private Object[][] obterDados() {
			List<MotivoPromocao> its = service.listarTodos();
			data                      = new Object[its.size()][columnNames.length];
			
			if(its != null && its.size() > 0){
			
				for (int i = 0; i < its.size(); i++) {
					MotivoPromocao g = its.get(i);
					data[i][0] = g.getIdMotivoPromocao().toString();
					data[i][1] = g.getNmMotivoPromocao();
...
//					pb = dao.obterProdutosById(Long.parseLong(key));
					...
					grau.setNmMotivoPromocao(descr);
					grau.setCdUsuaAtlz(getUsuario().getCodigoUsuario());
					grau.setDhAtlz(Util.hojeDataHoraTimesTamp());
					service.salvar(grau);
