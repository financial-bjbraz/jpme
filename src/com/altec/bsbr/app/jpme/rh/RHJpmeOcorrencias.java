...
public class RHJpmeOcorrencias extends BaseScreenPainel implements BaseScreenFactory{
...
//		add(pnl1, BorderLayout.NORTH);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource() == btBuscar){
			...
			if(!Util.isBlankOrNull(textoInclusao.getText())){
				Ocorrencias grau = new Ocorrencias();
				...
				grau.setNmOcorrecia(textoInclusao.getText());
	...
	public class MyTableModel extends AbstractTableModel {
		...
		private Object[][] obterDados() {
			List<Ocorrencias> its = service.listarTodos();
			data                      = new Object[its.size()][columnNames.length];
			
			if(its != null && its.size() > 0){
			
				for (int i = 0; i < its.size(); i++) {
					Ocorrencias g = its.get(i);
					data[i][0] = g.getIdOcorrencia().toString();
					data[i][1] = g.getNmOcorrecia();
