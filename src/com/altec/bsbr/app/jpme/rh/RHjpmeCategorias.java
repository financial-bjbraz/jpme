...
public class RHjpmeCategorias extends BaseScreenPainel implements BaseScreenFactory{
...
//		add(pnl1, BorderLayout.NORTH);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource() == btBuscar){
			...
			if(!Util.isBlankOrNull(textoInclusao.getText())){
				Categorias grau = new Categorias();
				...
				grau.setNmCategoria(textoInclusao.getText());
	...
	public class MyTableModel extends AbstractTableModel {
		...
		private Object[][] obterDados() {
			List<Categorias> its = service.listarTodos();
			data                      = new Object[its.size()][columnNames.length];
			
			if(its != null && its.size() > 0){
			
				for (int i = 0; i < its.size(); i++) {
					Categorias g = its.get(i);
					data[i][0] = g.getIdCategoria().toString();
					data[i][1] = g.getNmCategoria();
