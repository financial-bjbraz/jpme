...
public class RHjpmeNacionalidade extends BaseScreenPainel implements BaseScreenFactory{
...
//		add(pnl1, BorderLayout.NORTH);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource() == btBuscar){
			...
			if(!Util.isBlankOrNull(textoInclusao.getText())){
				Nacionalidade grau = new Nacionalidade();
				...
				grau.setCdPais(1);
				grau.setNmNaci(textoInclusao.getText());
	...
	public class MyTableModel extends AbstractTableModel {
		...
		private Object[][] obterDados() {
			List<Nacionalidade> its   = service.listarTodos("");
			data                      = new Object[its.size()][columnNames.length];
			
			if(its != null && its.size() > 0){
			
				for (int i = 0; i < its.size(); i++) {
					Nacionalidade g = its.get(i);
					data[i][0] = g.getCdNaci().toString();
					data[i][1] = g.getNmNaci();
