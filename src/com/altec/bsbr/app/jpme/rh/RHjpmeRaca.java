...
public class RHjpmeRaca extends BaseScreenPainel implements BaseScreenFactory{
...
//		add(pnl1, BorderLayout.NORTH);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource() == btBuscar){
			...
			if(!Util.isBlankOrNull(textoInclusao.getText())){
				Raca grau = new Raca();
				grau.setIdUsrIncl(getUsuario().getCodigoUsuario());
				...
				grau.setNmRaca(textoInclusao.getText());
				service.salvar(grau);
				constroiPainelResultados();
	...
	public class MyTableModel extends AbstractTableModel {
		...
		private Object[][] obterDados() {
			List<Raca> its = service.listarTodos();
			...
			if(its != null && its.size() > 0){
				...
				for (int i = 0; i < its.size(); i++) {
					Raca g = its.get(i);
					data[i][0] = g.getIdRaca().toString();
					data[i][1] = g.getNmRaca();
					data[i][2] = "Ativo";
